...
class RegisterAccount(APIView):
    ...
    """
    throttle_scope = 'anonim'

    # Регистрируем пользователя через POST
    def post(self, request, *args, **kwargs):
        # проверяем аргументы пользователя
        if {'first_name', 'last_name', 'email', 'password', 'organization', 'position'}.issubset(request.data):
            ...
            try:
                ...
                error_array = []
                # проверяем noinspection PyTypeChecker
        ...
        return JsonResponse({'Status': False, 'Errors': 'Укажите все необходимые аргументы '})
...
class ConfirmAccount(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response({'Status': False, 'Errors': 'Укажите необходимые аргументы'},
...
class BasketView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return JsonResponse({'Status': False, 'Errors': 'Укажите необходимые аргументы'})
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if items_sting:
            ...
            if objects_deleted:
                ...
                return JsonResponse({'Status': True, 'Всего объектов удалено': deleted_count})
        return JsonResponse({'Status': False, 'Errors': 'Укажите необходимые аргументы'})
...
class PartnerState(APIView):
    ...
    """
    throttle_scope = 'user'
    # Получаем статус получения заказов у магазина
    def get(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return Response({'Status': False, 'Error': 'Login required'}, status=status.HTTP_403_FORBIDDEN)
    ...
    # Изменяем статус получения заказов у магазина
    def post(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return Response({'Status': False, 'Error': 'Log in required'}, status=status.HTTP_403_FORBIDDEN)
        if request.user.type != 'shop':
